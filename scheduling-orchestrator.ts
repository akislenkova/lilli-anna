...
//
// This orchestrator handles steps 1-7. Step 8 uses
// Epic's SendMessage API (see epic-messaging.ts).
// ─────────────────────────────────────────────────────────

import { allocateTime } from "../../allocator/time-allocator";
import { classifyPatientInput } from "../../classifier/intent-classifier";
import {
    ClassificationResult,
    TimeAllocation,
    VisitCategory
} from "../../types";
import { EpicFhirClient, FhirAppointment, FhirCondition } from "./epic-fhir-client";
import { EXAMPLE_VISIT_TYPE_MAP, VisitTypeMapping, resolveVisitType } from "./visit-type-mapping";
...
export async function getSchedulingRecommendation(
  ...
  const classification = classifyPatientInput(patientInput);
  ...
  try {
    ...
    if (conditionsMatchReason && classification.primaryIntent.category !== VisitCategory.CHRONIC_MANAGEMENT) {
      // The patient might not have explicitly said "diabetes management"
      // but if they have diabetes on their problem list and mention anything
      // related, we should flag it
      classification.flags.push({
        type: "complex_history" as any,
...
export async function bookWithRecommendation(
  ...
  // Build a comment that includes our AI reasoning
  // This gets attached to the appointment and visible in Epic
  ...
  // await sendToProviderInBasket(context.epicClient, recommendation);
...
function checkConditionMatch(
  ...
  // Map condition display names to keywords
  const conditionText = conditions
    .map(c => (c.code?.text || c.code?.coding?.[0]?.display || "").toLowerCase())
    .join(" ");

  // Check if any classified category keywords appear in conditions
